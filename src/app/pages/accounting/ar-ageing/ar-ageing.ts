...
export interface AgeingTotals extends AgeingCustomer {}
...
export class ArAgeingComponent implements OnInit, AfterViewInit {
  ...
  get anyExtendedData(): boolean {
    ...
    return this.report.customers.some(
      (c) =>
        c.days_90_120 +
          c.days_120_150 +
          c.days_150_180 +
  ...
  renderCharts() {
    ...
    if (this.barCanvas?.nativeElement && this.report.customers.length > 0) {
      ...
      this.barChart = new win.Chart(this.barCanvas.nativeElement, {
        ...
        data: {
          labels: top.map((c) =>
            c.customer_name.length > 16 ? c.customer_name.slice(0, 16) + '…' : c.customer_name
          ),
          datasets: [
  ...
  sendAllAlerts() {
    if (!this.report) return;
    alert(
      `Collection alerts logged for ${this.overdueCustomers.length} customer(s) with overdue balances.`
    );
  }
  ...
  riskClass(c: AgeingCustomer): string {
    const over90 =
      c.days_90_120 +
      c.days_120_150 +
      c.days_150_180 +
  ...
  get overdueCustomers(): AgeingCustomer[] {
    if (!this.report) return [];
    return this.report.customers.filter(
      (c) =>
        c.days_1_30 +
          c.days_31_60 +
          c.days_61_90 +
